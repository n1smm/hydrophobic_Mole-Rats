...
})({"70IJY":[function(require,module,exports) {
...
var _scriptMeyda = require("./scriptMeyda");
...
function applyEffect(context, width, height, averageDB) {
    ...
    for(let i = 0; i < data.length; i += 4){
        ...
        if (randomFactor > 1.1) data[i] += intensity * randomFactor; // Rosso
        else if (randomFactor < 0.9) data[i + 1] += intensity * randomFactor; // Verde
    }
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            img.onload = function() {
                ...
                if (averageDB > 0) applyEffect(ctx, img.width, img.height, averageDB);
...
stopButton.addEventListener("click", ()=>{
    if (isAnalyzerRunning) {
        (0, _scriptMeyda.analyzer).stop();
        (0, _scriptMeyda.powerSpectrumAnalyzer).stop();
        (0, _scriptMeyda.flatnessAnalyzer).stop();
        isAnalyzerRunning = false;
        stopButton.value = "Analyze";
    } else {
        (0, _scriptMeyda.analyzer).start();
        (0, _scriptMeyda.powerSpectrumAnalyzer).start();
        (0, _scriptMeyda.flatnessAnalyzer).start();
        isAnalyzerRunning = true;
        stopButton.value = "Stop";
        started = true;
    }
    if (started && !isAnalyzerRunning) {
        averageDB = (0, _scriptMeyda.totDB) / (0, _scriptMeyda.totDBCount);
...
},{"./scriptMeyda":"eYnSf"}],"eYnSf":[function(require,module,exports) {
//import { updateImage } from './script.js';
/* globals Meyda */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "totDB", ()=>totDB);
parcelHelpers.export(exports, "totDBCount", ()=>totDBCount);
parcelHelpers.export(exports, "analyzer", ()=>analyzer);
parcelHelpers.export(exports, "powerSpectrumAnalyzer", ()=>powerSpectrumAnalyzer);
parcelHelpers.export(exports, "flatnessAnalyzer", ()=>flatnessAnalyzer);
...
let averageDB = 0;
let analyzer;
let powerSpectrumAnalyzer;
let flatnessAnalyzer;
if (typeof Meyda === "undefined") console.log("Meyda could not be found! Have you included it?");
else {
...
document.body.addEventListener("touchstart", touchStarted, false); //export { averageDB };
...
},{}]},["70IJY","6rimH"], "6rimH", "parcelRequire24b6")
